...
pub mod pallet {
	...
	pub struct MyInput<T: Config> {
		field_1: u32,
		field_2: T::Hash,
		field_3: T::AccountId,
	}

	#[derive(Clone, Eq, PartialEq, Debug, TypeInfo, Encode, Decode, MaxEncodedLen)]
	pub struct MyOtherInput<Hash, AccountId> {
		field_1: u32,
		field_2: Hash,
		field_3: AccountId,
	}
	...
	#[pallet::storage]
	type MyStorage<T: Config> = StorageValue<_, MyInput<T>>;
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(2)]
		#[pallet::weight(Weight::default())]
		pub fn input_something(
			_origin: OriginFor<T>,
			_input: MyInput<T>,
			_input_2: MyOtherInput<T::Hash, T::AccountId>,
		) -> DispatchResult {
			Ok(())
		}
	}
